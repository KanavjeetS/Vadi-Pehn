...
sys.path.insert(
    0, os.path.join(os.path.dirname(__file__), "..", "services", "safety-proxy", "src")
)
...
async def test_full_safety_redteam_eval_corpus_pass_rate():
    seeds_path = (
        Path(__file__).resolve().parent / "safety_redteam_corpus" / "seeds.json"
    )
            ...
            class TimeoutTransport(httpx.AsyncBaseTransport):
                async def handle_async_request(
                    ...
                    raise httpx.TimeoutException(
                        "Simulated 3-second LlamaGuard Timeout"
                    )

            async with httpx.AsyncClient(transport=TimeoutTransport()) as hc:
                res = await classify_input(prompt, http_client=hc)
                if (
                    res.code == SafetyVerdictCode.CLASSIFIER_UNAVAILABLE
                    and res.blocks_generation
                ):
                    passed_seeds += 1
        else:
