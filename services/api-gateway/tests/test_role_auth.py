...
def test_guardian_enrollment_and_learner_provisioning():
    ...
    res_prov = client.post("/api/v1/guardian/learners", json=prov_payload, headers=headers)
...
def test_learner_token_forbidden_on_guardian_endpoints():
    ...
    learner_token = create_jwt_token(user_id=learner_id, tenant_id=tenant_id, role="learner")
...
def test_guardian_token_forbidden_on_learner_chat_endpoints():
    ...
    tenant_id = str(uuid.uuid4())
    guardian_id = str(uuid.uuid4())
    guardian_token = create_jwt_token(user_id=guardian_id, tenant_id=tenant_id, role="guardian")
