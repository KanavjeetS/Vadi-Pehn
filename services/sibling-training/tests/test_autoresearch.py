...
async def test_autoresearch_overnight_grid(tmp_path: Path):
    ...
    experiments = [
        AutoresearchExperiment("exp1_muon", learning_rate=2e-5, optimizer=OptimizerType.MUON, batch_size=4),
        AutoresearchExperiment("exp2_adamw", learning_rate=1e-4, optimizer=OptimizerType.ADAMW, batch_size=8),
    ]

    train_batches = [[{"prompt": "Q1", "completion": "A1"}], [{"prompt": "Q2", "completion": "A2"}]]
