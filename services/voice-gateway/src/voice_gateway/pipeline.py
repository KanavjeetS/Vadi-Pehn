...
from services.abstractions import SafetyClient, SafetyVerdict, SafetyVerdictCode, MockSafetyClient
from voice_gateway.abstractions import LiveKitRoomManager, STTService, TTSService, VADService
...
class VoiceTurnPipeline:
    ...
    async def stream_voice_turn(
        ...
        if request.text_fallback:
            ...
            transcript = await self.stt_service.transcribe(audio_bytes, language=request.language)
            audio_bytes = b""
        ...
        if input_verdict.blocks_generation:
            ...
            audio_buf = await self.tts_service.synthesize(fallback_text, language=request.language)
        ...
        sentence_chunks = split_into_sentence_chunks(draft_reply)
        ...
        for chunk in sentence_chunks:
            ...
            if output_verdict.blocks_generation:
                ...
                t_tts = time.perf_counter()
                audio_buf = await self.tts_service.synthesize(safe_fallback, language=request.language)
                ...
                t_tts = time.perf_counter()
                audio_buf = await self.tts_service.synthesize(chunk, language=request.language)
                if not report.tts_first_chunk_ms:
                    report.tts_first_chunk_ms = (time.perf_counter() - t_tts) * 1000.0
                report.total_e2e_ms = (time.perf_counter() - start_e2e) * 1000.0
                yield (chunk, audio_buf, report)

        report.safety_output_per_chunk_ms = total_output_safety_ms / max(len(sentence_chunks), 1)
    ...
    async def execute_voice_turn(self, request: VoiceTurnRequest) -> VoiceTurnResponse:
        ...
        # Publish synthesized audio stream to LiveKit room
        room_name = f"room_{request.session_id}"
        await self.room_manager.stream_audio_to_room(room_name, combined_audio)
        ...
        return VoiceTurnResponse(
            ...
            safety_verdict=SafetyVerdictCode.SAFE.value,
            latency_report=last_report,
            audio_response_base64=base64.b64encode(combined_audio).decode("utf-8") if combined_audio else None,
        )
