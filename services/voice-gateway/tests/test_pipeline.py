...
from voice_gateway.pipeline import (
    VoiceTurnPipeline,
...
def mock_pipeline() -> VoiceTurnPipeline:
    return VoiceTurnPipeline(
        ...
        stt_service=MockSTTService(
            transcript_to_return="I want to learn robotics and build drones."
        ),
...
@pytest.mark.asyncio
async def test_stream_sentence_chunks_emits_provider_deltas() -> None:
    async def provider():
        yield "First sentence. "
        yield "Second sentence"

    chunks = [chunk async for chunk in stream_sentence_chunks(provider())]
...
@pytest.mark.asyncio
async def test_classifier_unavailable_blocks_unsafe_input_fallback_audio() -> None:
    """A blocked input fallback must also pass output safety before TTS."""
    safety_client = MockSafetyClient(
    ...
    request = VoiceTurnRequest(
        session_id="session_classifier_unavailable",
        tenant_id=uuid4(),
        learner_id=uuid4(),
        age_band=2,
        text_fallback="synthetic classifier timeout case",
    )

    response = await pipeline.execute_voice_turn(request)

    assert response.safety_verdict == SafetyVerdictCode.CLASSIFIER_UNAVAILABLE.value
    assert response.audio_response_base64 is None
    assert tts_service.last_synthesized_text == ""
